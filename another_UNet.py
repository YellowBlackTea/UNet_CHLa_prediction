...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=8, depth=5,
                 ...
                 transfo_field=False, nb_inputs=2, residual_block=False):
        """
        Arguments:
...
if __name__ == "__main__":
    ...
    model = UNet(1, depth=5, in_channels=7, merge_mode='concat', activation='ReLU', transfo_field=False, residual_block=True)
    model.to(device=1)
    ...
    # output_names = ['yhat']
    a=torch.Tensor(np.random.random(((1,7,360,100)))).to(device=1)
